'''here is to take 2 dices and its result frequencies'''
...
result = []
max_sizes = dice_1.size_num + dice_2.size_num
'''when increase the '''
for roll_num in range(num_roll):
    result_dice = dice_1.roll_dice() + dice_2.roll_dice()
    result.append(result_dice)
print(len(result))

'''From the point your get for each roll, then count the frequencies of the result'''
frequencies = []
for point in range(2,max_sizes+1):
...
x_value = list(range(2,max_sizes))
...
offline.plot({'data':data, 'layout':my_layout}, filename='roll_dice_2.html')
